...
export const CATEGORIES: Category[] = [
  ...
  },
  {
    ...
    icon: "🗝",
    blurb:
      "The shadow war. Scouts are the whole intelligence arm AND the only defence against spies — a watch that outweighs the knives sent at it turns them back outright, and shortens whatever the rest manage to plant. Spies are the whole destruction arm, and a party fills from the hired before it touches your own. Both spend from one scarce pool of spy turns, so watching a rival and robbing them compete for the same purse.",
    groups: ["Covert"],
  },
  {
    key: "endgame",
...
export const SCALARS: ScalarMeta[] = [
  ...
  { key: "MERC_UPKEEP_GOLD_PER_TURN", label: "Mercenary upkeep (retired)", unit: "gold / turn", group: "Mercenaries", value: C.MERC_UPKEEP_GOLD_PER_TURN, min: 0, step: 1, desc: "RETIRED — held at 0 and no longer read by the tick. Sellswords are bought outright with a one-time price; what limits them is the cap tying their number to the regulars of the same arm, not a wage." },
  // Market
  ...
  { key: "COMBAT_TEMPO", label: "Strike bite", unit: "% of power per exchange", group: "Combat", value: C.COMBAT_TEMPO, min: 0.01, max: 1, step: 0.01, pct: true, desc: "THE combat dial. A battle is one exchange down the order of battle, and this is how much of an arm's power lands in it. Raise it and every strike kills more, so a war is decided in a handful of attacks; lower it and war becomes a long grind of many strikes. Touches troops only — never rams or trebuchets against masonry." },
  ...
  { key: "BOMBARD_INTENSITY", label: "Bombardment weight", unit: "\u00d7 volleys", group: "Combat", value: C.BOMBARD_INTENSITY, min: 1, step: 1, desc: "How heavily one bombardment lands, in volleys\u2019 worth of stone. Bombard is a single exchange like every other attack; this is the multiplier that makes a barrage a barrage." },
  ...
  { key: "REVENGE_BLOODLUST", label: "Revenge bloodlust", unit: "%", group: "Siege", value: C.REVENGE_BLOODLUST, min: 0, max: 2, step: 0.05, pct: true, desc: "Extra damage an AVENGING host deals, additive into the bonus pool. The only damage bonus in the game bought with nothing — no research, no building, no gold: you earn it by being hit. It matters more than it sounds, because a revenge cannot be answered with a yield and takes no loot, so this is the dial that decides whether the payment in dead regulars is ever actually collected." },
  { key: "SORTIE_TRIGGER_RATIO", label: "Sortie opens the gates at", unit: "× the screen", group: "Siege", value: C.SORTIE.TRIGGER_RATIO, min: 1, step: 0.1, desc: "How far the defender's footmen and cavalry must outweigh the attacker's screen before the gates open at all. Clearing this only buys the CHANCE to fight: the screen still has to be beaten in the open before anything reaches the engines." },
  { key: "SORTIE_MIN_STAMINA", label: "Garrison sallies only above", unit: "stamina", group: "Siege", value: C.SORTIE.MIN_STAMINA, min: 0, max: 100, step: 5, desc: "A garrison below this will not open its gates — tired men hold the wall and wait. Sits far above the mercy floor (30) on purpose: between the two a defender is well enough to FIGHT but not well enough to SALLY, which makes the sortie a first answer rather than a last one. A garrison being ground down stops riding out long before it stops resisting." },
  { key: "SORTIE_MIN_SCREEN", label: "…and only with a screen of", unit: "% of the hire cap", group: "Siege", value: C.SORTIE.MIN_SCREEN, min: 0, max: 1, step: 0.05, pct: true, desc: "Hired footmen and cavalry the garrison must still have, against what the cap allows, before it will ride out. Sellswords absorb the merc share of every blow aimed at their rank, so charging without them means paying for the charge in your own population. Archer and engineer sellswords are not counted — they stay behind the wall and cannot cushion a charge they are nowhere near." },
  { key: "SORTIE_FOOTMEN_HOLD", label: "Footmen draw off", unit: "riders each", group: "Siege", value: C.SORTIE.FOOTMEN_HOLD, min: 0, step: 0.5, desc: "How many charging riders ONE of the besieger's footmen draws off and occupies. Counted in men, not power: a hundred footmen tie up two hundred riders whoever those riders are. Every rider occupied is a rider not reaching the archers, the crews and the engines — so a screen protects the siege train by being numerous, and cheap troops out front are a real answer to a sortie." },
  { key: "SORTIE_CAVALRY_HOLD", label: "Cavalry draw off", unit: "riders each", group: "Siege", value: C.SORTIE.CAVALRY_HOLD, min: 0, step: 0.5, desc: "…and how many one of the besieger's OWN cavalry draw off, counter-charging what spills past the footmen. More than footmen manage, because meeting a charge with a charge is the thing horse are for. Only riders neither arm can occupy reach the engines." },
  { key: "SORTIE_CAVALRY_BONUS", label: "Cavalry in the open", unit: "%", group: "Siege", value: C.SORTIE.CAVALRY_BONUS, min: 0, max: 2, step: 0.05, pct: true, desc: "What horse gain riding out onto open ground — the entire reason the standing order exists. Cavalry are worthless on a parapet and murderous off it, so a cavalry-heavy defender sorties and a footman-heavy one holds." },
  { key: "SORTIE_ARCHER_MELEE", label: "Archers in the rear-guard clash", unit: "% of power", group: "Siege", value: C.SORTIE.ARCHER_MELEE, min: 0, max: 1, step: 0.05, pct: true, desc: "What an archer manages with horse already inside the siege lines. A bow is a poor weapon at knife range, but the man holding it is still a soldier. Read ONLY here — archers shoot at full power from a parapet." },
  { key: "SORTIE_ENGINE_SHARE", label: "Breakthrough aimed at the engines", unit: "%", group: "Siege", value: C.SORTIE.ENGINE_SHARE, min: 0, max: 1, step: 0.05, pct: true, desc: "Of what a breakthrough lands, the share that goes into the siege park rather than the archers and engineers defending it. Riders come for the engines — but they have to go through people to reach them, and what they spend on people is not spent on trebuchets." },
  { key: "ENGINEER_POWER", label: "Engineer power", unit: "power", group: "Siege", value: C.UNIT_POWER.engineer.power, min: 0, step: 5, desc: "What an engineer manages when a sortie reaches the siege lines and they pick up whatever is to hand — about half a light footman. Read ONLY by the rear-guard clash: engineers never march out to attack." },
  { key: "DAMAGE_TAKEN_ENGINEER", label: "Damage an engineer takes", unit: "% of a blow", group: "Siege", value: C.DAMAGE_TAKEN.engineer, min: 0, max: 1, step: 0.05, pct: true, desc: "Not skill — flight. Engineers scatter into the lines the moment horse are among the engines, and a rider chasing one is a rider not burning a trebuchet. Without some relief here a single breakthrough would end a siege by killing every crew on the field. Level with cavalry and worse than a drilled footman: running beats standing to shoot, and loses to a shield." },
  ...
  { key: "MEDICINE_RECOVER_PER_LEVEL", label: "Medicine — recovered per level", unit: "% of the fallen", group: "Mercenaries", value: C.MEDICINE.RECOVER_PER_LEVEL, min: 0, max: 1, step: 0.01, pct: true, desc: "Share of the critically wounded the field hospital pulls off alive, per research level. BOTH armies at their own level, attacking or defending, and your own dead and the hired counted as SEPARATE budgets — pooled, a screen dying in bulk bought back every regular who fell, which is your people being saved by other men dying. Regulars are treated first when the granary runs short." },
  { key: "MEDICINE_MIN_PER_LEVEL", label: "Medicine — floor per level", unit: "heads", group: "Mercenaries", value: C.MEDICINE.MIN_PER_LEVEL, min: 0, step: 1, desc: "Guaranteed heads per level even when the percentage rounds to nothing. Without a floor the field does visibly nothing in small skirmishes, which is most of what a new player fights." },
  { key: "MEDICINE_FOOD_PER_RECOVERY", label: "Medicine — food per head", unit: "food", group: "Mercenaries", value: C.MEDICINE.FOOD_PER_RECOVERY, min: 0, step: 5, desc: "Stores the surgeons spend per man saved. Treatment is partial when the granary is short — enough for three of five saves three — and it may draw on the vault." },
  ...
  { key: "INTERCEPT_MAX", label: "Interception ceiling", unit: "%", group: "Covert", value: C.INTERCEPTION.MAX, min: 0, max: 1, step: 0.05, pct: true, desc: "Ceiling on how many of a party the watch can lay hands on in one night. It caps the GRAB, not the outcome — a watch that outweighs the knives outright turns them back whatever this reads, and half of everyone grabbed slips free besides." },
  { key: "ENGINEER_CAP", label: "Engine crews cap", unit: "% of population", group: "Covert", value: C.ENGINEER_CAP, min: 0, max: 1, step: 0.01, pct: true, desc: "Crews of your OWN, on the same rule as the shadow arms and twice the room, because a siege park is the larger undertaking. This is what really bounds a siege train: engines you cannot man are engines you may not keep." },
  { key: "SIEGE_STOCK_RATIO", label: "Engines kept per crewed one", unit: "×", group: "Siege", value: C.SIEGE_STOCK_RATIO, min: 1, step: 0.5, desc: "How many engines of a type you may hold against what your engineers could actually man, counted SEPARATELY for the offensive train and the defensive battery. Without it the yard is a bank: engines are bought with resources and sold on the black market, so a realm at a punishing tax rate could pour income into trebuchets it never meant to crew and break them out again later, beyond the reach of tax or raid." },
  { key: "PREPARATION_PER_MULTIPLE", label: "Preparation per extra multiple of turns", unit: "%", group: "Covert", value: C.PREPARATION.PER_EXTRA_MULTIPLE, min: 0, max: 2, step: 0.05, pct: true, desc: "An operation's turn cost is a MINIMUM. Commit more and the surplus buys the knives time — reconnoitred routes, bribed gatemen, a night of their choosing — which multiplies what the party is worth against the watch and at the work. Doubling the turns on a raid that failed half the time stops it failing, for a third fewer men lost. Spies only: a scouting party already spends its turns on people." },
  { key: "PREPARATION_MAX", label: "…capped at", unit: "%", group: "Covert", value: C.PREPARATION.MAX, min: 0, max: 3, step: 0.25, pct: true, desc: "Patience is worth a great deal and then it is worth nothing more. Past this you are waiting where you could be working, and the spy-turn budget is the same one the rangers draw from." },
  { key: "REGULAR_SPY_POWER", label: "Your own spy vs a hired one", unit: "×", group: "Covert", value: C.REGULAR_SPY_POWER, min: 1, step: 0.5, desc: "What one of your own knives is worth against a bought one, against the watch AND at the work. Written this way round on purpose: the sellsword is the baseline and your own are the multiplier, so this RAISES what a regular achieves rather than taxing a hireling. It is a temptation, and it has teeth because a party fills from the HIRED first — the only way to get your own people into an operation is to send enough to exhaust the hire pool, which is also exactly how you put them in danger." },
  { key: "SLIP_THROUGH", label: "Grabbed agents who get away", unit: "%", group: "Covert", value: C.SLIP_THROUGH, min: 0, max: 1, step: 0.05, pct: true, desc: "Being laid hands on is not being taken. One number for everyone — a hand closing on a shoulder in the dark does not first ask who was paying. This is the single largest thing standing between a spy corps and annihilation: without it a heavy watch simply deleted an infiltration. Agents who slip still finish the job, and a raid where they all get away leaves no body to name you." },
  { key: "REFUSAL_RATE", label: "Guild master refuses above", unit: "% grabbed", group: "Covert", value: C.REFUSAL_RATE, min: 0, max: 1, step: 0.05, pct: true, desc: "Agents are people. Told to walk into a realm so watched that this share of them would be seized, they decline — and declining is free, since it is judged on nominal odds before a turn is spent. It reads the GRAB rather than the losses, because slipping caps actual losses near 45% and any threshold above that could never fire. It is also the only signal in the game about how heavy another realm's watch is: no scout operation reports it." },
  { key: "SCOUT_TURNS_PER_RANGER", label: "Turns per ranger sent", unit: "spy turns", group: "Covert", value: C.SCOUT_MISSION.TURNS_PER_SCOUT, min: 0, step: 0.5, desc: "A flat rate whatever they are looking at. The extra turns a deeper mission demands come from needing more PEOPLE, not from a second rate — so for rangers, turns and bodies are one dial rather than two." },
  { key: "SCOUT_POP_SCALE", label: "Population that doubles a scout mission", unit: "souls", group: "Covert", value: C.SCOUT_MISSION.POP_SCALE, min: 100, step: 500, desc: "Counting a giant's granaries is an expedition; counting a neighbour's is an afternoon. A mission needs its base head-count times (1 + their population ÷ this), so reading a great power costs most of a day's covert budget and reading a neighbour costs an hour." },
  { key: "SCOUT_PATHFINDING_RELIEF", label: "Pathfinding cuts the party by", unit: "% / level", group: "Covert", value: C.SCOUT_MISSION.PATHFINDING_RELIEF, min: 0, max: 0.5, step: 0.05, pct: true, desc: "The field's only job on OFFENCE. It promised '+20% to recon' for a long time while scout power was computed for the defender alone and never for your own missions; it now buys down the rangers a party needs, which is the same promise made true." },
  { key: "SCOUT_MIN_FILL", label: "Below this a party cannot finish", unit: "% of what it needs", group: "Covert", value: C.SCOUT_MISSION.MIN_FILL, min: 0, max: 1, step: 0.05, pct: true, desc: "Rangers are never caught, so the price of coming up short is a vaguer answer rather than a funeral — but there has to be a floor, or there is no such thing as sending too few. Below it they come home with nothing but the bill." },
  { key: "RECON_FUZZ", label: "How wide a short-handed report reads", unit: "± % at zero fill", group: "Covert", value: C.RECON_FUZZ, min: 0, max: 2, step: 0.05, pct: true, desc: "Every figure a party brings back narrows toward the truth as it is better manned, and reads exactly when fully manned. The range is NOT centred on the truth — a centred one would let anyone read the midpoint and have perfect intelligence for half the rangers, which is the whole thing this exists to stop." },
  { key: "INFILTRATION_SCALE", label: "Survivors for a full-length effect", unit: "spies", group: "Covert", value: C.COVERT_EFFECTS.INFILTRATION_SCALE, min: 1, step: 5, desc: "How many knives must get through to buy the full duration of unrest or doubt. Below it the effect is proportionally shorter. It used to be flat — one spy over the wall and a hundred bought exactly the same day of ruin, which made sizing an infiltration pointless." },
  { key: "MIN_DURATION_FRACTION", label: "Shortest a landed effect can run", unit: "% of its base", group: "Covert", value: C.COVERT_EFFECTS.MIN_DURATION_FRACTION, min: 0, max: 1, step: 0.05, pct: true, desc: "A defender only just outmatched was taking fourteen minutes of unrest: an alarm in the inbox and nothing wrong by the time they looked, which reads as a bug rather than a near-miss. With a floor there are only two outcomes and both are legible — it bounced, or you felt it." },
  { key: "COVERT_CAP_PER_ARM", label: "Spies (or scouts) cap", unit: "% of population", group: "Covert", value: C.COVERT_CAPS.PER_ARM, min: 0, max: 1, step: 0.01, pct: true, desc: "Ceiling on each arm as a share of your WHOLE PEOPLE — civilians and regular military, sellswords never. It bounds how many of your OWN may be knives; hired ones sit on top under the ordinary hire ratio. Six hundred rangers therefore means a realm of twelve thousand souls, and a realm that large deserves to be near untouchable." },
...
export const REF_TABLES: RefTable[] = [
  ...
  {
    key: "dodge",
    title: "How much of a blow each arm takes",
    group: "Units",
    desc: "The other half of survivability, and the one nobody sees on a unit card. Damage aimed at an arm is multiplied by this before it touches anybody, so a footman behind a shield turns four blows in five while an archer — hands full of bow, no shield, standing still to shoot — turns three in five. It applies to hired and raised alike: a bought archer is still an archer. Engineers carry the flight rate rather than a skill one; they scatter, and a rider chasing one is a rider not burning a trebuchet.",
    headers: ["Unit", "Takes", "Turns aside"],
    rows: (["footman", "cavalry", "archer", "engineer"] as const).map((u) => [
      u,
      `${Math.round(C.DAMAGE_TAKEN[u] * 100)}%`,
      `${Math.round((1 - C.DAMAGE_TAKEN[u]) * 100)}%`,
    ]),
  },
  {
    key: "siege",
  ...
  },
  {
    ...
    group: "Combat",
    desc: "Experience is a LEDGER, not a pool: credited for the men you kill, debited for the men you lose, with no ceiling. Picking on the much weaker costs you points outright; beating somebody stronger pays double or treble.",
    headers: ["Situation", "Points"],
    rows: [
      ["Per enemy casualty (either side)", `+${C.EXPERIENCE.PER_CASUALTY}`],
      ["Per enemy REGULAR \u2014 attacker only", `+${C.EXPERIENCE.ATTACKER_PER_REGULAR} more (withheld from defenders so an army cannot be fed to a friend)`],
      ["Per regular of your OWN lost", `\u2212${C.EXPERIENCE.PER_REGULAR_LOST}, never scaled`],
      ["Discharging instead of dying", `\u00d7${C.EXPERIENCE.DISCHARGE_FACTOR} of that`],
      ["Won \u2014 attacking / defending", `\u00d7${C.EXPERIENCE.WON_ATTACK} / \u00d7${C.EXPERIENCE.WON_DEFENCE}`],
      ["Lost the field", `\u00d7${C.EXPERIENCE.LOST}`],
      ["They surrendered", "nothing, to either side \u2014 nobody fought"],
      ["Punching up (\u00d71.25\u20131.75 score)", "\u00d72 \u2192 \u00d73"],
      ["Bullying (under \u00d70.5 score)", "NEGATIVE \u2014 the more you massacre, the more you lose"],
      ["Ceiling per battle", `+${C.EXPERIENCE.MAX_PER_BATTLE.toLocaleString("en-US")}`],
      ["Attack refused above", `\u00d7${C.ATTACK_REFUSAL_RATIO} score ratio`],
      ["Buys +100% power and health at", `${C.EXPERIENCE.POINTS_FOR_DOUBLE.toLocaleString("en-US")} points \u2014 and keeps going past it`],
    ],
  },
  {
    key: "loot",
    ...
    rows: [
      ["Raid, won (peace)", `up to ${Math.round(C.LOOT.PEACE_CEILING * 100)}% of exposed GOODS`],
      ["Castle, won (peace)", `up to ${Math.round(C.LOOT.PEACE_CEILING * 100)}% of unvaulted GOLD`],
      ["Either, won (clan war)", `${Math.round(C.LOOT.WAR_SHARE * 100)}% \u2014 everything outside the vault`],
      ["They laid down arms", `\u00d7${C.LOOT.YIELD_FACTOR} of what fighting would have cost \u2014 war or peace`],
      ["Rolled band, before the ceiling", `${Math.round(C.LOOT.RAID_WIN.min * 100)}\u2013${Math.round(C.LOOT.RAID_WIN.max * 100)}%`],
