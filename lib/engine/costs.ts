...
export function buildingCost(id: BuildingId, targetLevel: number): Cost {
  ...
  // The war-works. Ore-heavy and on the gentlest ladder in the game, because
  // their return is a flat +5% a level — see WARWORKS_COST.
  if (WARWORKS_BUILDING_IDS.includes(id)) {
    const m = evalCurve(WARWORKS_COST_CURVE, targetLevel);
    const b = WARWORKS_COST.BASE;
    return {
      gold: Math.round(b.gold * m),
      wood: Math.round(b.wood * m),
      stone: Math.round(b.stone * m),
